...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(operator, MacroProcedure):
            expr = operator.apply_macro(rest, operator.env)
            return scheme_eval(expr, env)
        def eval_with_env(x):
            return scheme_eval(x, env)
        operands = rest.map(eval_with_env)
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        while formals is not nil and vals is not nil:
            ...
            vals = vals.second
        if vals is not nil or formals is not nil:
            raise SchemeError
        return child_frame
        # END PROBLEM 11
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 12
        new_frame = Frame(self.env).make_child_frame(self.formals, args)
        return new_frame
        # END PROBLEM 12
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        lambda_form = LambdaProcedure(target.second, expressions.second, env)
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    form_checker = lambda expr: check_form(expr, 1, 2)
    bindings.map(form_checker)
    keys = bindings.map(lambda expr: expr.first)
    check_formals(keys)
    values = bindings.map(lambda expr: scheme_eval(expr.second.first, env))
    new_frame = Frame(env).make_child_frame(keys, values)
    return new_frame
    # END PROBLEM 15
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN PROBLEM 10
        macro_form = MacroProcedure(target.second, expressions.second, env)
        env.bindings[target.first] = macro_form
        return target.first
    else:
        bad_target = target.first if isinstance(target, Pair) else target
        raise SchemeError('non-symbol: {0}'.format(bad_target))
